...
int explMsg(char *msg){
   ...
   if(strcmp(type,"seat/")==0){//座次消息
     ...
     LOG2F(filename,"座次信息处理完毕");
		...
		LOG2F(filename,"盲注信息处理完毕");
   } else  if ( strcmp (type, "hold/" )==0){//手牌消息
			LOG2F(filename,"接受到手牌信息");
			...
			p = strtok(p, "\n" );//除去最后的结束标志
			p = p + strlen (p)+1;
			LOG2F(filename,"手牌信息处理完毕");
   } else  if ( strcmp (type, "inquire/" )==0){//询问消息
			LOG2F(filename,"接受到询问信息");
			while(1)
			{
				p = strtok(p, "\n" );
				sscanf (p,"%s",temp);
			...
			*/
			...
			sendMsg(ALL_IN,0);
			LOG2F(filename,"询问信息处理完毕");
			
   } else  if ( strcmp (type, "flop/" )==0){//公牌消息
		LOG2F(filename,"接受到公牌信息");
		...
		for(int i=0;i<3;i++){
			...
			p = strtok(p, "\n" );//除去最后的结束标志
			p = p + strlen (p)+1;
			LOG2F(filename,"公牌信息处理完毕");
   } else  if ( strcmp (type, "turn/" )==0){//转牌消息
		LOG2F(filename,"接受到转牌信息");
...
int conServer(char ** argv)
{
   ...
   while(connect(sock,(struct sockaddr*)&serverAddr,sizeof(struct sockaddr))<0){
     ...
     sprintf(c,"连接失败:%d",times);
	++times;);
     LOG2F(filename,c);
   }
   LOG2F(filename,"success con");
   char c[100];
   sprintf(c,"reg: %s %s \n",argv[5],"Shader");
